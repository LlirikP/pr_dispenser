...
const getTeamNameByID = `-- name: GetTeamNameByID :one
SELECT teamname
FROM teams
WHERE id = $1
`

func (q *Queries) GetTeamNameByID(ctx context.Context, id string) (string, error) {
	row := q.db.QueryRowContext(ctx, getTeamNameByID, id)
	var teamname string
